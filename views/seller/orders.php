<?php $activePage = 'orders'; $headerTitle = 'Orders'; include VIEW_PATH . '/layouts/admin_layout.php'; ?>
<div style="margin-bottom:20px;display:flex;gap:8px;flex-wrap:wrap;">
    <?php foreach (['' => 'All', 'pending' => 'Pending', 'confirmed' => 'Confirmed', 'shipped' => 'Shipped', 'delivered' => 'Delivered'] as $val => $label): ?>
    <a href="<?= base_url('seller/orders' . ($val ? '?status='.$val : '')) ?>" class="btn btn-sm <?= ($statusFilter ?? '') === $val ? 'btn-primary' : 'btn-outline' ?>"><?= $label ?></a>
    <?php endforeach; ?>
</div>
<div class="dashboard-card">
    <div class="table-responsive">
        <table class="data-table">
            ...
            <tbody>
                <?php if (empty($orders)): ?>
                <tr><td colspan="6" style="text-align:center;padding:40px;color:var(--text-muted);">No orders found</td></tr>
                <?php else: foreach ($orders as $o): ?>
                <tr style="cursor:pointer;" onclick="window.location='<?= base_url('seller/orders/' . $o['id']) ?>'">
